...
particle filter, five exercises over four 180-minute sessions (ICP and MCL), the nodes that run them and
the graders that mark them. ASY2 practicum *Intelligente Robotik*.

The library is numpy. ROS 2 is only needed for `ros2 launch` and `ros2 run`; grading needs neither — and
neither do three of the five exercises, which run on a laptop with numpy and no simulator at all.
...
The three exercises that need no robot are graded by the tool of the same shape, in about a second:

```bash
python3 tools/lab_check.py                    # E1, E2 and E4, on your files, with the numbers
python3 tools/lab_check.py e2_icp_pair --verbose
```
...
## 6 · The exercises: five, over four sessions

Two topics — ICP scan matching and Monte-Carlo localisation — two 180-minute sessions each, and one file to
write for each exercise. Three of the five need nothing but numpy: they are graded on seeded synthetic data in
seconds, so a group runs the command that produces their mark as often as they like during the session, and the
two on the robot are graded by the simulator's own grader.

<!-- lab-exercises:begin — generated by tools/make_lab_docs.py from config/exercises_localization.json -->
| visit | exercise | what you write | points | the command that grades it |
|---|---|---|---|---|
| I1 | **E1** The nearest neighbour inside ICP | `nn_template.py` | 20 | `python3 tools/lab_check.py e1_nn` |
| I1 | **E2** One model, one scene — and the σ that goes with them | `icp_pair_template.py` | 30 | `python3 tools/lab_check.py e2_icp_pair` |
| I2 | **E3** Scan matching as the odometry of a real robot | `icp_odom_template.py` | 30 | `./tools/run_lab.sh grade --task icp_odom_production --controller student/icp_odom_template.py --headless` |
| M1 | **E4** Generating the particles of a Monte Carlo localiser | `particles_template.py` | 50 | `python3 tools/lab_check.py e4_particles` |
| M2 | **E5** The complete localiser on the robot | `mcl_template.py` | 130 | `./tools/run_lab.sh grade --task mcl_production,mcl_wide,mcl_budget,mcl_dirty --controller student/mcl_template.py --headless` |

Four 180-minute sessions, **260 points**. **I1** 180 min ICP, visit 1 of 2 → E1 + E2 · **I2** 180 min ICP, visit 2 of 2 → E3 · **M1** 180 min Monte Carlo localisation, visit 1 of 2 → E4 · **M2** 180 min Monte Carlo localisation, visit 2 of 2 → E5. Three of the five exercises (E1, E2, E4) are graded by `python3 tools/lab_check.py` on seeded synthetic data, need no simulator and can be re-run as often as you like during the session; the two on the robot (E3, E5) are graded by the simulator's own grader. The sheet for each one is `docs/handout/lab-<id>.md`, the long-form task text with the numbers behind it is [`docs/exercises.md`](docs/exercises.md), what a tutor prepares is [`docs/lab-plan.md`](docs/lab-plan.md), and every threshold in the table is a measurement listed, with the run that produced it, in [`docs/verification.md`](docs/verification.md).
<!-- lab-exercises:end -->

Sheets, one per exercise: [`docs/handout/`](docs/handout/), and the same text as GitHub issues in
[`.github/issues/`](.github/issues) — `.github/publish_issues.sh` opens them with `gh`, and is a paste-list
without it. The long-form version of each task, with the numbers behind it, is
[`docs/exercises.md`](docs/exercises.md); what happens in which session, and what a tutor prepares, is
[`docs/lab-plan.md`](docs/lab-plan.md).

## 7 · The four graded MCL tasks
...
```
ohm_localization/   gridmap · mcl · icp · synth · exercises · paths · hall · lab · mcl_node · drive_node · icp_odom_node · map_server_node · view · rviz_config
examples/           four small programs, one command each — see examples/README.md
config/             tasks_localization.json (the four graded MCL tasks: thresholds, drives, hints) and
                    exercises_localization.json (the five exercises: criteria, points, thresholds, sheet text)
student/            one file per exercise, each with one TODO — the table in §6 — and FAILURE.md
solution/           the reference each threshold was measured against, one file per exercise
launch/             mcl.launch.py · mcl.rviz (the RViz view)
docs/               exercises · lab-plan · mcl · icp · verification · handout/ (the sheets)
tools/              run_lab.sh, lab_check.py and the instruments the numbers in docs/ were measured with
test/               the suite; ./tools/check.sh runs it and the documents beside it
```

* [`docs/exercises.md`](docs/exercises.md) — every task with its checkpoints and the numbers behind it
* [`docs/lab-plan.md`](docs/lab-plan.md) — the four sessions: what happens when, and what a tutor prepares
