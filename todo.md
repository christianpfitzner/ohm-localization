...
| the LIDAR fan is drawn off the walls, further off the longer the drive goes on | RESOLVED | Not the filter: TF. The simulator publishes `map -> <robot>/odom` as the identity so that the Kalman lab's `map -> base_link` *is* the drifting odometry, and a scan has nothing but TF to say where it is. `mcl_node` now publishes that edge from the estimate (`tf:=localizer`, the AMCL convention, the mode the simulator reserves for it via `tf.tree: slam`), which takes the mean error of the drawn scan from 0.204 m to 0.016 m while `kf/pose` stays where it was. The frame follows the publisher — `hall` in that tree, `map` in the other — and `/<robot>/odom/path` draws the drift by hand because a corrected odometry trail would land on the estimate. §17 |
...
- Two `ros2 launch` runs on one machine share one DDS domain, and both publishing `<robot>/odom` gives that frame two parents: TF then drops the older stamp at 20 Hz and the scan jumps metres. Nothing here prevents it — a `ROS_DOMAIN_ID` per group would — so it is a check before the symptom, not a fix. §17
